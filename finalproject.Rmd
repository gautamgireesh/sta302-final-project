...
p_prime0 <- length(full_model$coefficients)
summary(full_model)
SSres0<-deviance(full_model)
AIC0<-n*log(SSres0) -n*log(n) + 2*p_prime0
print(AIC0)
...
no_outliers_american <- american_data[-influential_obs, ]
...
# 
# CD_american <- cooks.distance(model3)
# CD_american <- round(CD_american, 5)
# sorted_CD <-rev(sort(CD_american))
# print(sorted_CD)
# mean_CD <- mean(CD_list)
# threshold <- 4*mean_CD
# 
# influential<-names(sorted_CD)[sorted_CD > threshold]
# influential_vector<-c()
# for(i in influential){
#   influential_vector <- append(influential_vector, as.numeric(i))
# }
# 
# no_influential_american <- american_data[-influential_vector, ]

new_model3<-lm(quiz4~centered_quiz_avg, data=no_outliers_american)
...
asian_p_prime1 <- length(full_asian_model$coefficients)
summary(full_asian_model)
asian_SSres1<-deviance(full_asian_model)
